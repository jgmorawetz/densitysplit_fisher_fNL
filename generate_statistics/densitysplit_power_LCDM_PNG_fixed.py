...
# IDENTICAL TO THE ORIGINAL DENSITY-SPLIT CODE BUT USING FIXED OVERDENSITY 
# THRESHOLDS INSTEAD TO ISOLATE ANY CONTRIBUTION COMING FROM VARYING IT
...
def get_quantile_positions(halo_positions, overdensity_thresholds, filter_type, filter_radius, 
                           n_randoms, boxsize, cellsize_ds, cellsize_lattice, query_type, 
                           resampler, interlacing, compensate, sim_index):
    """Retrieves quantile positions based on smoothed halo field (but this time
       using fixed overdensity thresholds).

    Args:
        halo_positions (array): The halo positions (dimension (N, 3)) where N is the 
                                number of halos.
        overdensity_thresholds (list): The overdensity thresholds to split into the
                                       separate density environments.
    ...
    # Splits query positions by density thresholds
...
def generate_statistics(halo_path, boxsize, snapnum, redshift, omega_m, min_mass, space, 
                        los_dirs, overdensity_thresholds, filter_type, filter_radius, n_randoms, nmesh, 
    ...
    for los in los_dirs:
        ...
        ds_quantiles = get_quantile_positions(
            halo_positions, overdensity_thresholds, filter_type, filter_radius, n_randoms, boxsize,
...
if __name__ == '__main__':
    ...
    overdensity_thresholds = [...]
    # Iterates through different combinations of the hyperparameters
    for hyperparameters in [(10, None, 'lattice')]:
        n_quantiles = len(overdensity_thresholds)+1
        filter_radius, n_randoms, query_type = hyperparameters
        # Iterates through the different parameter variations
        for variation in ['LC_m', 'LC_p']:
            ...
            output_folder = f'/home/jgmorawe/projects/rrg-wperciva/jgmorawe/results/quijote/ds_functions/power/{variation}_fixed'
            # Iterates through the different simulation indices
            for sim_index in sim_indices:
                halo_path = os.path.join(halo_folder, f'{sim_index}')
                generate_statistics(
                    halo_path, boxsize, snapnum, redshift, omega_m, min_mass, space, los_dirs, overdensity_thresholds, filter_type,
