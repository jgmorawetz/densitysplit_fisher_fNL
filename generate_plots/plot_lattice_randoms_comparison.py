...
if __name__ == '__main__':
    ...
    fig, ax = plt.subplots(3, 1, sharex=True, sharey=False, dpi=500, figsize=(5, 6), height_ratios=[2, 1, 2])
    fig.subplots_adjust(left=0.13, right=0.98, top=0.97, bottom=0.09, hspace=0.15, wspace=0.1)
    ax[0].plot(k_avg, k_avg**2*random_mean, '-', color='tab:blue', linewidth=1, label='Random')
    ax[0].fill_between(x=k_avg, y1=k_avg**2*(random_mean-random_std), y2=k_avg**2*(random_mean+random_std), alpha=0.2, color='tab:blue')
    ax[0].plot(k_avg, k_avg**2*lattice_mean, '-', color='tab:red', linewidth=1, label='Lattice')
    ax[0].fill_between(x=k_avg, y1=k_avg**2*(lattice_mean-lattice_std), y2=k_avg**2*(lattice_mean+lattice_std), alpha=0.2, color='tab:red')
    ...
    ax[0].set_ylabel(r'$k^2 P_0(k) \ [h^{-1}\mathrm{Mpc}]$')
    ax[1].set_ylabel(r'$\left<P_{\mathrm{lattice}}\right>/\left<P_{\mathrm{random}}\right>$')
    ax[2].set_ylabel(r'$\sigma_{\mathrm{lattice}}/\sigma_{\mathrm{random}}$')
    ax[0].set_xscale('log')
    ax[0].set_yscale('log')
    ax[0].legend(loc=1)
    fig.savefig(f'/home/jgmorawe/projects/rrg-wperciva/jgmorawe/results/quijote/plot_results/lattice_randoms_fiducial_power_DS{q}_l={l}.pdf')