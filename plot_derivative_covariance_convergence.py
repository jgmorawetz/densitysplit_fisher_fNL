...
if __name__ == '__main__':
    ...
    ncov_list = np.arange(1500, 15300, 300)
    ...
    fig, ax = plt.subplots(3, 2, dpi=400, figsize=(6*1.3,6*1.3))
    fig.subplots_adjust(hspace=0.15, wspace=0.2, left=0.08, right=0.97, top=0.96, bottom=0.07)
    ...
    for i in range(3):
        ...
        for j in range(len(param_list)):
            ...
            print(comb, param_list[j], 'raw: ', round(param_comb_data_deriv[-1], 4), 'corrected: ', round(param_comb_data_deriv[-1]*((1+best_fit_param)**0.5), 4))
    ax[0][0].set_xticks([])
    ax[1][0].set_xticks([])
    ax[0][1].set_xticks([])
    ax[1][1].set_xticks([])
    ...
    ax[2][0].set_title('Halo + DSC')
    ax[0][1].set_title('Halo')
    ax[1][1].set_title('DSC')
    ax[2][1].set_title('Halo + DSC')
    ax[0][0].legend()
    ...
    fig.savefig('/home/jgmorawe/projects/rrg-wperciva/jgmorawe/results/quijote/plot_results/covariance_derivative_convergence_baseline.png')